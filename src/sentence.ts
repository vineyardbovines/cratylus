...
export function buildSentence(stringArr: string[]): string {
  ...
  const filteredPunctuation = endPunctuation.filter((punct) =>
    stringArr.includes(punct)
  );

  const withPunctuation = filteredPunctuation.length
    ? stringArr
        .join(" ")
        .replace(` ${filteredPunctuation[0]}`, filteredPunctuation[0])
    : [...stringArr, "."].join(" ").replace(" .", ".");

  return withPunctuation.replace(sentenceRegex, (str) => str.toUpperCase());
}
